...
display_memory_usage() {
	...
	center_text "$(color_text "bg_bright_cyan" " Memory: $memory_usage ($memory_percentage)")"
	center_text "$(color_text "bg_bright_yellow" "Swap: $swap_usage ($swap_percentage)")"
}

# Function to display system info like kernel uptime and related information
display_system_info() {
	...
	center_text "$(color_text "bg_magenta" " Logged-in User: $user")"
	...
	center_text "$(color_text "bg_cyan" " Resolution: $screen")"
...
display_cpu_info() {
	...
	center_text "$(color_text "bg_bright_magenta" " CPU: $cpu_model")"
	center_text "$(color_text "bg_blue" " CPU Cores: $cpu_cores")"
	center_text "$(color_text "bg_bright_cyan" " CPU Threads: $cpu_threads")"
}

# Function to display temperature information
display_temperature_info() {
	local cpu_temp=$(sensors | grep 'Package id 0:' | awk '{print $4}')
	center_text "$(color_text "bg_red" "🌡️ CPU Temperature: $cpu_temp")"
}
...
display_network_info() {
	...
	local dns=$(cat /etc/resolv.conf | grep nameserver | awk '{print $2}')
